...
namespace pisomkaZaver
{
    public partial class Form1 : Form
    {
        ...
        private void vypis()
        {
            ...
            {
                if (item.Cislo.Contains(textBox1.Text)) { break; }
                if (item.Meno.Contains(textBox2.Text)) { break; }
                if (item.Tieda.Contains(textBox3.Text)) { break; }
                if (item.Status.Contains(textBox4.Text)) { break; }

                richTextBox1.AppendText(item.Cislo + "  " + item.Meno + "  " + item.Tieda + "  " + item.Status + "\n"); 
            }
        }
